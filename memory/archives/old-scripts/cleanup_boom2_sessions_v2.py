...
def cleanup_old_sessions():
    ...
    # 7일 전 시간 (밀리초) - 7일 = 7 * 24 * 60 * 60 * 1000 = 604,800,000 ms
    ...
    print(f"삭제 기준: {datetime.fromtimestamp(seven_days_ago/1000).strftime('%Y-%m-%d %H:%M:%S')} 이전")
    ...
    for session_key, session_data in data.items():
        ...
        if updated_at < seven_days_ago:
            ...
            print(f"🗑️ 삭제 대상: {session_key[:50]}... (마지막 업데이트: {last_update}, {days_old:.1f}일 전)")
        else:
            to_keep.append(session_key)
...
if __name__ == "__main__":
    print("🧹 붐2 오래된 세션 정리 시작 (7일 이상된 세션만)")
    print("=" * 60)
